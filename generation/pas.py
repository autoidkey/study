...
def prev_arg_check_kai(clauses,cla,string,bra_f):
	#clauses [[[文節説明],[形態素]],....]
	#print("prev")
	#係り元文節番号
	arg_num = int(cla[0][1])
	...
	for i in reversed(range(arg_num)) :
		...
		if rela[0] == cla[0][1] :
			...
			#再帰
			string = prev_arg_check(clauses,r_cla,string,bra_f)
	else :
		return string
#述語の操作
def pre_check(pre) :
	...
	for c in cut :
		if c in pre :
			if c == "て" :
				if not "ていない" in pre and not "てる" in pre:
					pres = pre.split(c)
					#print("pres:",pres)
					pre = pres[0] 
			else :#あればカット
